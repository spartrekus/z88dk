...
#define _Z80_ONLY(x)		(!(opt_cpu() & (CPU_Z80|CPU_Z80_ZXN)) ? \
								...
								(x))
#define _EXCEPT_Z80(x)		((opt_cpu() & (CPU_Z80|CPU_Z80_ZXN)) ? \
								(error_illegal_ident(), 0) : \
								(x))
#define _Z180_ONLY(x)		(!(opt_cpu() & CPU_Z180) ? \
								(error_illegal_ident(), 0) : \
								(x))
#define _RCM2000_ONLY(x)	(!(opt_cpu() & CPU_R2K) ? \
								(error_illegal_ident(), 0) : \
								(x))
#define _RCM3000_ONLY(x)	(!(opt_cpu() & CPU_R3K) ? \
								(error_illegal_ident(), 0) : \
								(x))

#define _ZILOG_ONLY(x)		(!(opt_cpu() & CPU_ZILOG) ? \
								(error_illegal_ident(), 0) : \
								(x))
#define _RABBIT_ONLY(x)		(!(opt_cpu() & CPU_RABBIT) ? \
								(error_illegal_ident(), 0) : \
								(x))
...
#define P_AF	0
#define P_SP	0
#define P_IX	(opt_swap_ix_iy() ? 0xFD00 : 0xDD00)
#define P_IY	(opt_swap_ix_iy() ? 0xDD00 : 0xFD00)
...
#define Z80_EX_IND_SP_HL	((opt_cpu() & CPU_RABBIT) ? 0xED54 : 0xE3)
...
#define Z80_MLT(dd)			((opt_cpu() & CPU_Z80) ? \
								(error_illegal_ident(), 0) : \
								(opt_cpu() & CPU_RABBIT) && (dd) == REG_SP ? \
									(error_illegal_ident(), 0) : \
									0xED4C + ((dd) << 4))
