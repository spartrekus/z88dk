...
void test_expand_env_vars()
{
	...
	string text = "x${TEST1}y${TEST2}z$(TEST3)%TEST4%";
	string ret = expand_env_vars(text);
	IS(text, "x${TEST1}y${TEST2}z$(TEST3)%TEST4%");
	IS(ret, "xyzworldhello");

	auto_expand_env_vars(text);
	IS(text, "xyzworldhello");
...
#ifdef _WIN32
	IS_STR(combine_path("", "b:file"), "file");
